...
def get_pratiche():
    conn = get_db()
    try:
        pratiche = conn.execute('''
            SELECT p.*, d.ragione_sociale AS ditta_nome
            FROM pratiche p
            LEFT JOIN ditte d ON p.ditta_id = d.id
    ...
    try:
        conn.execute('''
            UPDATE pratiche SET
                ...
                priorita=?, data_apertura=?, data_scadenza=?,
                data_chiusura=?, note=?
            WHERE id=?
        ''', (
            data.get('ditta_id'),
